...
def get_block(sock):
	data = recvall(sock,hearder_struct.size)
	(block_length,)=hearder_struct.unpack(data)
	return recvall(sock,block_length)

def put_block(sock,message):
	block_length = len(message)
	sock.send(header_srtuct.pack(block_length))
	sock.send(message)

def server(address):
	sock = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
	sock.socketopt(socket.SOL_SOCKET,socket.SO_REUSRADDR,1)
	sock.bind(address)
	sock.listen(1)
